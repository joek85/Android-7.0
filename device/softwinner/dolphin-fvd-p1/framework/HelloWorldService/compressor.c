...
void sf_simplecomp(sf_compressor_state_st *state, int rate, float pregain, float threshold, float knee, float ratio, float attack, float release){
	// sane defaults
...
void sf_advancecomp(sf_compressor_state_st *state, int rate, float pregain, float threshold,
	...
	int delaybufsize = (int) (rate * predelay);
	if (delaybufsize < 1){
		delaybufsize = 1;
	} else if (delaybufsize > SF_COMPRESSOR_MAXDELAY){
		delaybufsize = SF_COMPRESSOR_MAXDELAY;
		memset(state->delaybuf1, 0, sizeof(float) * delaybufsize);
		memset(state->delaybuf2, 0, sizeof(float) * delaybufsize);
	}
...
void sf_compressor_process(sf_compressor_state_st *state, int size, float *input1,float *input2, float *output1,float *output2){
	...
	float *delaybufL     = state->delaybuf1;
	float *delaybufR     = state->delaybuf2;
	...
	for (int ch = 0; ch < chunks; ch++){
		...
		for (int chi = 0; chi < samplesperchunk; chi++, samplepos++,
			...
			float inputL = input1[samplepos] * linearpregain;
			float inputR = input2[samplepos] * linearpregain;
			...
			// apply the gain
			output1[samplepos] = delaybufL[delayreadpos] * gain;
			output2[samplepos] = delaybufR[delayreadpos] * gain;
		}
	}
