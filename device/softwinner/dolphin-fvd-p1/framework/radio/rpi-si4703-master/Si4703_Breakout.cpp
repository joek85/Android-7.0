...
const char *resetdevice = "/sys/class/gpio_sw/PA20/data";
int resetfd;
struct timeval tp;
...
void Si4703_Breakout::setChannel(int channel)
{
        ...
        usleep(60000); //Wait 60ms - you can use or skip this delay
	...
	gettimeofday(&tp, NULL);
        ...
        while ( completedCount < 4 && millis() < endTime ) {
                ...
                if ( si4703_registers[STATUSRSSI] & (1<<RDSR) ) {
                        ...
                        usleep(40000); //Wait for the RDS bit to clear
                } else {
                        usleep(30000); //From AN230, using the polling method 40ms should be sufficient amount of time between checks
                }
        }
...
#define I2C_PEC1		0x0708
#define I2C_SLAVE1	0x0703
...
int Si4703_Breakout::si4703_init()
{
	...
	resetfd = open(resetdevice, O_RDWR); //read write
	if (resetfd < 0) {
        	printf("can't open reset device");
        	return -1;
        }
	...
	close(resetfd);
        ...
        if ( ioctl(si4703_fd, I2C_SLAVE1, SI4703) < 0 ) { //Set device address 0x10
                perror("Failed to aquire bus access and/or talk to slave");
		close(si4703_fd);
                return(FAIL);
        }

        if ( ioctl(si4703_fd, I2C_PEC1, 1) < 0 ) { //Enable "Packet Error Checking"
        ...
        usleep(500000); //Wait for clock to settle - from AN230 page 9
        ...
        usleep(110000); //Max powerup time, from datasheet page 13
